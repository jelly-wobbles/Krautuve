...
class SearchController extends Controller
{
    ...
    public function toCartAction(Request $request)
    {
        ...
        $shopArr = $em->getRepository('KTUShopBundle:Items')->findByItemsdetails( $item->getItemsdetails() );
        $temp = array();
        foreach($shopArr as $shopItem){
            if( $shopItem->getItemstatuses()->getStatus() == "Available" ){
                array_push( $temp, $shopItem );
            }
        }

        $shopArr = $temp;
        $shopAmount = sizeof($shopArr);

        $user = $em->getRepository('KTUShopBundle:Users')->findOneByid( $user->getId() );

        $cartArr = $em->getRepository('KTUShopBundle:Shoppingcarts')->findByusers( $user );

        if( $cartArr ){
            foreach( $cartArr as $cItem ){

                $thisDetailsID = $item->getItemsdetails()->getId();
                $cartItemID = $cItem->getItems()->getItemsdetails()->getId();

                if( $cartItemID == $thisDetailsID ){
                    $cartAmount = $cartAmount + 1;
                }
            }
        }

        if( ($item->getItemstatuses()->getStatus() != "Available") || !($item) || ($cartAmount >= $shopAmount) ){
